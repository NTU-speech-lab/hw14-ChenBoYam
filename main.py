# -*- coding: utf-8 -*-
"""Copy of hw14_life_long_learning.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1hH9nw_L24mqKMUzwBBSq6Q4-7UuWWmue
...
def train_process(model, optimizer, tasks, config):
  ...
  for task_id, task in enumerate(tasks):
    ...
    if config.mode == 'scp' and task_id > 0:
      ...
      for old_task in range(task_id): 
        old_dataloaders += [tasks[old_task].val_loader]
        scp = SCP(model, old_dataloaders, device)
        scp_tasks += [scp]
      while (total_epochs < config.num_epochs):
    ...
    def calculate_importance(self):
        ...
        for dataloader in self.dataloaders:
            for data in dataloader:
                ...
                ####################################################################################
                phi = output.mean(dim=0)
                phi_dim = phi.shape[0]
                ####################################################################################
                #####   2. 隨機 從 單位球殼 取樣 L 個 vector ξ #（ Hint: sample_spherical() ）      #####
                ####################################################################################
                psi = sample_spherical(npoints=self.L, ndim=phi_dim)
...
if __name__ == '__main__':
    ...
    #####                           TODO 區塊 （ PART 1 ）                   #####
    ############################################################################ 
    coeff_list = [5, 240 ,0 , 6]  ## 你需要在這 微調 lambda 參數, mas, ewc, baseline=0##  
    ############################################################################
    #####                           TODO 區塊 （ PART 1 ）                   #####
    ############################################################################ 
...
#####  TODO 區塊 （PART 2） #####
###############################
def plot_result_2(mode_list, task1, task2, task3, task4):
...
plot_result_2(mode_list,'SVHN','MNIST','USPS')